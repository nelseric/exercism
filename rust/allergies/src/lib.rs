...
impl Allergies {
  pub fn is_allergic_to(&self, allergen: &Allergen) -> bool {
    ...
    let index = allergens.iter().position(
      |x: &Allergen| x == allergen
    ).unwrap();
  ...
  pub fn allergies(&self) -> Vec<Allergen> {
    Allergies::allergens().into_iter().filter(
      |allergen| self.is_allergic_to(allergen)
    ).collect()
  }

  pub fn allergens() -> Vec<Allergen> {
